...
pub const Sudoku = struct {
    ...
    band_combinations: [3]u162 = .{ALL162} ** 3,
    ...
    pub fn solve(self: *Sudoku, cell_values: *const [81]u8) [81]u8 {
        ...
        assert(self.find_match(shortest_number));
    ...
    fn find_match(self: *Sudoku, number: usize) bool {
        self.numbers ^= BIT9[number];

        const numbers = self.numbers;
        const number_cells = self.number_cells;
        const band_combinations = self.band_combinations;
        ...
        while (biterate0 > 0) {
            ...
            if (number_band0 & POSSIBLES[band0_index] == POSSIBLES[band0_index] and (band_combinations[0] & NUMBER_COMBINATIONS[band0_index] > 0 or self.numbers == 0)) {
                var biterate1 = band_combinations[1] & BAND_COMBINATIONS[band0_index];
                while (biterate1 > 0) {
                    const band1_index = @ctz(biterate1);
                    if (number_band1 & POSSIBLES[band1_index] == POSSIBLES[band1_index] and (band_combinations[1] & NUMBER_COMBINATIONS[band1_index] > 0 or self.numbers == 0)) {
                        ...
                        while (biterate2 > 0) {
                            ...
                            if (number_band2 & POSSIBLES[band2_index] == POSSIBLES[band2_index] and (band_combinations[2] & NUMBER_COMBINATIONS[band2_index] > 0 or self.numbers == 0)) {
                                self.number_cells[number] = @as(u81, POSSIBLES[band0_index]) | @as(u81, POSSIBLES[band1_index]) << 27 | @as(u81, POSSIBLES[band2_index]) << 54;
                                if (self.numbers == 0) {
                                    return true;
                                }
                                var new_remove_from_others: [9]u81 = .{0} ** 9;
                                new_remove_from_others[number] = self.number_cells[number];
                                const shortest_number = self.remove_cells(new_remove_from_others);
                                self.band_combinations = .{
                                    band_combinations[0] & NUMBER_COMBINATIONS[band0_index],
                                    band_combinations[1] & NUMBER_COMBINATIONS[band1_index],
                                    band_combinations[2] & NUMBER_COMBINATIONS[band2_index],
                                };
                                if (self.is_sudoku and self.find_match(shortest_number)) {
                                    return true;
                                } else {
                                    self.is_sudoku = true;
